...
export default function PengaturanPage() {
  ...
  const [hapusGaji, setHapusGaji] = useState(false);

  // State untuk Hapus Karyawan
  const [isKaryawanModalOpen, setIsKaryawanModalOpen] = useState(false);
  const [confirmTextKaryawan, setConfirmTextKaryawan] = useState("");
  ...
  const REQUIRED_CONFIRM_TEXT_KARYAWAN = "HAPUS BUKU INDUK KARYAWAN";
  ...
  const handleOpenKaryawanModal = () => {
    setConfirmTextKaryawan("");
    setStatus({ type: "", message: "" });
    setIsKaryawanModalOpen(true);
  };

  const executeDeleteKaryawanData = async () => {
    if (confirmTextKaryawan !== REQUIRED_CONFIRM_TEXT_KARYAWAN) return;

    setIsDeleting(true);
    setStatus({
      type: "info",
      message: "Memulai proses penghapusan data karyawan...",
    });
    ...
    try {
      ...
      setStatus({
        type: "success",
        message: `Berhasil! Total ${totalDeleted} data karyawan telah dihapus permanen.`,
      });
      setIsKaryawanModalOpen(false);
    } catch (error) {
      console.error("Error menghapus data karyawan:", error);
      setStatus({
        type: "error",
        message: "Gagal menghapus data karyawan. Periksa koneksi internet Anda.",
      });
    } finally {
      setIsDeleting(false);
      ...
      <div className="bg-white rounded-xl border-2 border-papua-red/20 shadow-sm overflow-hidden mb-8">
        ...
        <div className="p-6 md:p-8 border-b border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="max-w-xl">
              <h3 className="text-base font-bold text-papua-primary mb-2">
                Hapus Data Buku Induk Karyawan
              </h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                Tindakan ini akan mengosongkan seluruh daftar karyawan pada sistem secara
                permanen. Data karyawan yang dihapus tidak dapat dipulihkan. Pastikan Anda sudah tidak membutuhkan data tersebut.
              </p>
            </div>

            <button
              onClick={handleOpenKaryawanModal}
              className="shrink-0 flex items-center gap-2 bg-white border-2 border-papua-red text-papua-red hover:bg-papua-red hover:text-white px-5 py-3 rounded-lg text-sm font-bold transition-all shadow-sm"
            >
              <Trash2 className="w-5 h-5" /> Hapus Data Karyawan
            </button>
          </div>
        </div>
      ...
      {isKaryawanModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all">
            <div className="bg-papua-red p-6 flex flex-col items-center text-center relative">
              <button
                onClick={() => setIsKaryawanModalOpen(false)}
                className="absolute top-4 right-4 text-papua-red/30 hover:text-white transition-colors"
                disabled={isDeleting}
              >
                <X className="w-6 h-6" />
            ...
            <div className="p-6 space-y-5 bg-gray-50">
              <p className="text-sm text-gray-700 text-center font-medium leading-relaxed">
                Anda akan menghapus seluruh data pada{" "}
                <span className="font-black text-papua-red">Buku Induk Karyawan</span> secara{" "}
                <span className="font-black text-papua-red">PERMANEN</span>.
                Semua data karyawan akan hilang dan tidak dapat dipulihkan kembali.
              </p>
              ...
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-inner">
                ...
                <div className="text-center font-mono font-bold text-papua-red bg-papua-red/10 py-2 border border-papua-red/20 rounded mb-3 select-none">
                  {REQUIRED_CONFIRM_TEXT_KARYAWAN}
                </div>
                <input
                  type="text"
                  value={confirmTextKaryawan}
                  onChange={(e) => setConfirmTextKaryawan(e.target.value)}
                  placeholder="Ketik persis seperti kalimat di atas..."
                  className="w-full border-2 border-gray-300 rounded-lg px-4 py-3 outline-none focus:border-papua-red/100 focus:ring-2 focus:ring-papua-red/30 transition-all font-mono text-sm text-center"
                  autoComplete="off"
                  disabled={isDeleting}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  onClick={executeDeleteKaryawanData}
                  disabled={confirmTextKaryawan !== REQUIRED_CONFIRM_TEXT_KARYAWAN || isDeleting}
                  className="flex-1 flex justify-center items-center gap-2 px-4 py-3 bg-papua-red text-white rounded-lg font-bold hover:bg-papua-red transition-colors disabled:opacity-30 disabled:hover:bg-papua-red shadow-lg"
                >
                  {isDeleting ? (
                    <>
